...
Excel workbooks are generated and authorized by the backend. Subscription checkout hands the user to the bank URL returned by the backend and never introduces card fields into E-Növbə Web. Support, ownership transfer, customer blocking and Platform Admin resolution are explicit audited actions with nearby consequences and confirmation, rather than ordinary profile toggles.
