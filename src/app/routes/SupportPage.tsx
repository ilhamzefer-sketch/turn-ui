...
export function SupportPage() {
  ...
  return <div className="insight-page">
    ...
    <NotificationEvent tone="success" message={change.data ? `Müraciət #${change.data.id} qəbul edildi. Status: ${change.data.status}` : null} />
    <NotificationEvent tone="error" message={change.error?.message ?? null} />
    <NotificationEvent tone="success" message={deletion.data ? `Silinmə müraciəti #${deletion.data.id} qəbul edildi.` : null} />
    <NotificationEvent tone="error" message={deletion.error?.message ?? null} />
    <div className="support-grid"><section className="insight-panel"><p className="eyebrow">Telefon dəyişikliyi</p><h2>Yeni nömrə üçün müraciət</h2>{change.data ? null : <form className="operation-form" onSubmit={(e) => { e.preventDefault(); change.mutate(); }}><PhoneField label="Yeni telefon nömrəsi" value={phone} onChange={(e) => setPhone(e.target.value)} required /><TextAreaField label="Dəyişiklik səbəbi" value={reason} onChange={(e) => setReason(e.target.value)} required /><p className="form-note">Yeni nömrə yalnız manual yoxlamadan sonra hesabınıza tətbiq ediləcək.</p><Button type="submit" loading={change.isPending} disabled={!isLocalPhone(phone) || !reason.trim()}>Telefon dəyişikliyi göndər</Button></form>}</section>
      <section className="insight-panel insight-panel--danger"><p className="eyebrow">Hesabın silinməsi</p><h2>Manual silinmə müraciəti</h2><p>Aktiv biznesin əsas sahibisinizsə, əvvəlcə sahibliyi başqa aktiv administratora ötürməlisiniz.</p>{deletion.data ? null : <>{confirmDelete ? <div className="confirm-box"><p>Bu əməliyyat dərhal silmir; Platform Support yoxladıqdan sonra hesab anonimləşdirilə bilər.</p><Button loading={deletion.isPending} onClick={() => deletion.mutate()}>Bəli, müraciəti göndər</Button><Button variant="quiet" onClick={() => setConfirmDelete(false)}>Geri qayıt</Button></div> : <Button variant="secondary" onClick={() => setConfirmDelete(true)}>Hesabın silinməsini istə</Button>}</>}</section></div>
  </div>;
}
