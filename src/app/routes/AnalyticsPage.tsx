...
import { daysAgo, dateInput, analyticsRangeError, weekdayLabel } from "../../features/step-six/formatters";
...
export function AnalyticsPage({ scope }: { scope: "business" | "room" }) {
  ...
  const query = useQuery({ queryKey: [scope, id, "analytics", from, to], queryFn: () => scope === "business" ? stepSixApi.businessAnalytics(id, from, to) : stepSixApi.roomAnalytics(id, from, to), enabled: validId && !rangeError });
  const [downloading, setDownloading] = useState(false);
  usePageMeta("Əməliyyat analitikası — NövbəTime", "Canlı növbə və planlı rezervasiya üzrə əməliyyat göstəriciləri.");
  const report = validId && !rangeError ? query.data : undefined;
  const maxRoom = Math.max(1, ...(report?.rooms.map((room) => room.liveEntries + room.plannedBookings) ?? [1]));
  const download = async () => { setDownloadError(null); setDownloading(true); try { if (scope === "business") await stepSixApi.downloadBusinessAnalytics(id, from, to); else await stepSixApi.downloadRoomAnalytics(id, from, to); } catch (error) { setDownloadError(error instanceof Error ? error.message : "Hesabat endirilə bilmədi. Yenidən cəhd edin."); } finally { setDownloading(false); } };
  return <div className="insight-page">
    ...
    {!validId ? <p role="alert">Hesabat identifikatoru düzgün deyil.</p> : rangeError ? <p role="alert">{rangeError}</p> : query.isPending ? <div className="management-state" role="status">Hesabat hazırlanır…</div> : query.isError ? null : report ? <>
