...
export function RoomOwnersSection({ room }: { room: ManagedRoom }) {
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: async () => {
      setSelectedUserId("");
      setSuccessMessage("Otaq owner dəvəti göndərildi. İcazə istifadəçi qəbul etdikdən sonra aktiv olacaq.");
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: async () => {
      setSuccessMessage("Otaq owner icazəsi ləğv edildi.");
      await Promise.all([
  ...
  return (
    <div className="room-section-stack">
      ...
      {error ? <div className="form-alert" role="alert">{apiMessage(error, "Owner əməliyyatı tamamlanmadı.")}</div> : null}

      <section className="management-panel" aria-labelledby="room-owner-title">
        <div className="section-heading">
          <div><p className="eyebrow">Ortaq idarəetmə</p><h2 id="room-owner-title">Otaq owner-ləri</h2></div>
          <p>Bütün aktiv owner-lər eyni növbə, qrafik və otaq ayarlarını idarə edir.</p>
        </div>

        {canManageAssignments ? (
          <div className="owner-invite-bar">
            <SelectField label="Komandadan owner seçin" value={selectedUserId} onChange={(event) => setSelectedUserId(event.target.value)}>
            ...
            <Button disabled={!selectedUserId} loading={assignMutation.isPending} onClick={() => assignMutation.mutate(Number(selectedUserId))}>Owner dəvəti göndər</Button>
            {candidates.length === 0 ? <p>Uyğun yeni komanda üzvü yoxdur. Əvvəl biznes komandasına telefonla dəvət göndərin.</p> : null}
          </div>
        ) : (
          <div className="info-note">
            {room.businessId
              ? "Owner əlavə etmək və ya başqa owner-in icazəsini ləğv etmək yalnız biznesin əsas sahibi və administratorları üçündür."
              : "Fərdi otağın əsas owner-i hesab sahibidir. Şəxsi otağa əlavə owner təyin edilmir."}
          </div>
        )}

        {assignmentsQuery.isPending ? <p role="status">Owner-lər açılır…</p> : (
                  ...
                  <div className="owner-list__actions">
                    ...
                    {!isCurrentUser && canManageAssignments ? (
                      <Button
                        ...
                        onClick={() => {
                          if (window.confirm(`${assignment.firstName} ${assignment.lastName} üçün otaq owner icazəsi ləğv edilsin? Son aktiv owner silinərsə otaq avtomatik dayandırılacaq.`)) revokeMutation.mutate(assignment.id);
                        }}
      ...
      {canManageAssignments && room.businessId ? (
        <aside className="room-helper-card">
          <div><strong>Telefon siyahıda yoxdur?</strong><p>Əvvəl istifadəçini biznes komandasına əlavə edin, sonra bu otağa owner kimi dəvət edin.</p></div>
