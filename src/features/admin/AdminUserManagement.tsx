...
function UserDetail({ user, onBack }: { user: AdminUser; onBack: () => void }) {
  ...
  const credit = useMutation({
    mutationFn: () => {
      const signature = JSON.stringify([user.id, Number(amount), reason.trim()]);
      if (attempt.current?.signature !== signature) attempt.current = { signature, key: idempotencyKey() };
      return stepSixApi.adminCreditCoins(user.id, Number(amount), reason.trim(), attempt.current.key);
    },
    onSuccess: async () => {
  ...
  return <div className="admin-user-detail"><div className="admin-user-detail__toolbar"><Button variant="secondary" onClick={onBack}>← İstifadəçi siyahısı</Button><span className="admin-user-detail__id">İstifadəçi #{user.id}</span></div><article className="admin-record"><div className="admin-record__summary"><div><h3>{user.firstName} {user.lastName}</h3><p>{user.phone}</p></div><div className="admin-balance"><span>Coin balansı</span><strong>{user.coinBalance.toLocaleString("az-AZ")}</strong></div></div><dl className="admin-record__meta"><div><dt>Vəziyyət</dt><dd>{user.status === "ACTIVE" ? "Aktiv" : user.status}</dd></div><div><dt>Fırıldaq sayı</dt><dd>{user.confirmedWalletFraudCount}</dd></div><div><dt>Qeydiyyat</dt><dd>{new Date(user.createdAt).toLocaleDateString("az-AZ")}</dd></div></dl>{user.confirmedWalletFraudCount >= 3 ? <p className="admin-form-message" role="status">Bu istifadəçinin yeni çekləri yalnız admin təsdiqindən sonra balansa əlavə olunur.</p> : null}<form className="admin-action-form" onSubmit={(event) => { event.preventDefault(); if (valid) setConfirming(true); }}><TextField disabled={credit.isPending} label="Əlavə ediləcək coin" type="number" min="1" max="1000000" inputMode="numeric" value={amount} onChange={(event) => { setAmount(event.target.value); setConfirming(false); }} /><TextAreaField disabled={credit.isPending} label="Əlavə səbəbi" value={reason} onChange={(event) => { setReason(event.target.value); setConfirming(false); }} />{credit.error ? <p className="admin-form-message" role="alert">{credit.error.message}</p> : null}{credit.isSuccess ? <p className="admin-form-message admin-form-message--success" role="status">Coin əlavə edildi. Yeni balans: {credit.data.balanceAfter} coin.</p> : null}{confirming ? <div className="admin-confirm" role="alert"><p><strong>{Number(amount).toLocaleString("az-AZ")} coin</strong> {user.firstName} {user.lastName} hesabına əlavə ediləcək.</p><div><Button loading={credit.isPending} onClick={() => credit.mutate()}>Əlavəni təsdiqlə</Button><Button variant="quiet" disabled={credit.isPending} onClick={() => setConfirming(false)}>Ləğv et</Button></div></div> : <Button type="submit" disabled={!valid}>Coin əlavə et</Button>}</form><AdminUserPasswordForm user={user} /></article></div>;
}
