...
describe("NotificationProvider", () => {
  it("shows success confirmations in an accessible popup and keeps them dismissed", async () => {
    ...
    const dialog = await screen.findByRole("alertdialog", { name: "Əməliyyat tamamlandı" });
    expect(dialog).toHaveTextContent("Növbə rejiminin ayarları saxlanıldı.");
  ...
  it("shows only the latest notification when success and error states overlap", async () => {
    ...
    const dialog = await screen.findByRole("alertdialog", { name: "Əməliyyat tamamlanmadı" });
    expect(dialog).toHaveTextContent("Canlı növbə otağı üçün reset qaydası seçilməlidir.");
